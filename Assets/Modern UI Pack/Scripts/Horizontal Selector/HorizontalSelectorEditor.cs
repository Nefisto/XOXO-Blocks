...
namespace Michsky.MUIP
{
    ...
    public class HorizontalSelectorEditor : Editor
    {
        ...
        private void OnEnable()
        {
            ...
            try
            {
                tempUIM = hsTarget.GetComponent<UIManagerHSelector>();
            }
            catch { }
        ...
        public override void OnInspectorGUI()
        {
            ...
            switch (currentTab)
            {
                ...
                case 2:
                    ...
                    GUILayout.BeginVertical(EditorStyles.helpBox);
                    GUILayout.Space(-3);
                    enableIndicators.boolValue = MUIPEditorHandler.DrawTogglePlain(enableIndicators.boolValue,
                        customSkin, "Enable Indicators");
                    GUILayout.Space(3);
                    GUILayout.BeginHorizontal();

                    if (enableIndicators.boolValue)
                    {
                        if (hsTarget.indicatorObject == null)
                            EditorGUILayout.HelpBox(
                                "'Enable Indicators' is enabled but 'Indicator Object' is not assigned. Go to Resources tab and assign the correct variable.",
                                MessageType.Error);
                        if (hsTarget.indicatorParent == null)
                            EditorGUILayout.HelpBox(
                                "'Enable Indicators' is enabled but 'Indicator Parent' is not assigned. Go to Resources tab and assign the correct variable.",
                                MessageType.Error);
                        else
                    ...
                    invertAnimation.boolValue =
                        MUIPEditorHandler.DrawToggle(invertAnimation.boolValue, customSkin, "Invert Animation");
                    loopSelection.boolValue =
                        MUIPEditorHandler.DrawToggle(loopSelection.boolValue, customSkin, "Loop Selection");
                    GUI.enabled = true;
