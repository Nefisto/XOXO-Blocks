...
namespace Michsky.MUIP
{
    public class ContextMenuSubMenu : MonoBehaviour, IPointerClickHandler, IPointerEnterHandler, IPointerExitHandler
    {
        ...
        public void OnPointerEnter (PointerEventData eventData)
        {
            ...
            for (var i = 0; i < cmContent.contexItems[subMenuIndex].subMenuItems.Count; ++i)
            {
                var nulLVariable = false;

                if (cmContent.contexItems[subMenuIndex].subMenuItems[i].contextItemType
                    == ContextMenuContent.ContextItemType.Button
                    && cmManager.contextButton != null)
                {
                    selectedItem = cmManager.contextButton;
                }
                else if (cmContent.contexItems[subMenuIndex].subMenuItems[i].contextItemType
                         == ContextMenuContent.ContextItemType.Separator
                         && cmManager.contextSeparator != null)
                {
                ...
                if (nulLVariable == false)
                {
                        ...
                        == ContextMenuContent.ContextItemType.Button)
                    {
                        ...
                        itemButton.onClick.AddListener(cmContent.contexItems[subMenuIndex]
                            .subMenuItems[i].onClick.Invoke);
