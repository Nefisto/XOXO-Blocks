    ...
    public class NotificationManager : MonoBehaviour, IPointerClickHandler
    {
        ...
        private void Awake()
        {
            ...
            if (useStacking)
                try
                {
                    transform.GetComponentInParent<NotificationStacking>().AddToStack(this);
                }
                catch
                {
                    Debug.LogError(
                        "<b>[Notification]</b> 'Stacking' is enabled but 'Notification Stacking' cannot be found in parent.",
                        this);
                }
        }
        ...
        public void UpdateUI()
        {
            ...
            if (slideDirection == SlideDirection.Left)
            {
                ...
                transform.GetChild(0).transform.localScale = new Vector3(-1,
                    transform.GetChild(0).transform.localScale.y, transform.GetChild(0).transform.localScale.z);
            }
