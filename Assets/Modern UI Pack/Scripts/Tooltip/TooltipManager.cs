...
namespace Michsky.MUIP
{
    public class TooltipManager : MonoBehaviour
    {
        ...
        private void Awake()
        {
            var sourceRect = gameObject.GetComponent<RectTransform>();
            ...
            tooltipContent.GetComponent<RectTransform>().pivot =
                new Vector2(0f, tooltipContent.GetComponent<RectTransform>().pivot.y);
            tooltipContent.GetComponent<RectTransform>().pivot =
                new Vector2(tooltipContent.GetComponent<RectTransform>().pivot.x, 0f);
        ...
        private void CheckForPosition()
        {
            ...
            if (mainCanvas.renderMode == RenderMode.ScreenSpaceCamera || mainCanvas.renderMode == RenderMode.WorldSpace)
            {
                RectTransformUtility.ScreenPointToLocalPointInRectangle(
                    tooltipRect.parent.GetComponent<RectTransform>(), cursorPos, targetCamera, out var outPoint);
                tooltipRect.localPosition = outPoint;

                if (transitionMode == TransitionMode.Damp)
                    tooltipContent.transform.localPosition = Vector3.SmoothDamp(tooltipContent.transform.localPosition,
                        contentPos, ref tooltipVelocity, tooltipSmoothness, dampSpeed * 1000, Time.unscaledDeltaTime);
                else
            ...
            else if (mainCanvas.renderMode == RenderMode.ScreenSpaceOverlay)
            {
                ...
                if (transitionMode == TransitionMode.Damp)
                    tooltipContent.transform.position = Vector3.SmoothDamp(tooltipContent.transform.position,
                        cursorPos + contentPos, ref tooltipVelocity, tooltipSmoothness, dampSpeed * 1000,
