...
namespace Michsky.MUIP
{
    ...
    public class SliderManagerEditor : Editor
    {
        ...
        private void OnEnable()
        {
            ...
            try
            {
                tempUIM = sTarget.GetComponent<UIManagerSlider>();
            }
            catch { }
            ...
            switch (currentTab)
            {
                case 0:
                    ...
                    if (sTarget.mainSlider != null)
                    {
                        ...
                        EditorGUILayout.LabelField(new GUIContent("Min Value"), customSkin.FindStyle("Text"),
                            GUILayout.Width(120));
                        ...
                        EditorGUILayout.LabelField(new GUIContent("Max Value"), customSkin.FindStyle("Text"),
                            GUILayout.Width(120));
                    ...
                    else
                    {
                        EditorGUILayout.HelpBox(
                            "'Main Slider' is not assigned. Go to Resources tab and assign the correct variable.",
                            MessageType.Error);
                    }
                ...
                case 2:
                    ...
                    usePercent.boolValue =
                        MUIPEditorHandler.DrawToggle(usePercent.boolValue, customSkin, "Use Percent");
                    showValue.boolValue = MUIPEditorHandler.DrawToggle(showValue.boolValue, customSkin, "Show Label");
                    showPopupValue.boolValue =
                        MUIPEditorHandler.DrawToggle(showPopupValue.boolValue, customSkin, "Show Popup Label");
                    useRoundValue.boolValue =
                        MUIPEditorHandler.DrawToggle(useRoundValue.boolValue, customSkin, "Use Round Value");
                    invokeOnAwake.boolValue =
                        MUIPEditorHandler.DrawToggle(invokeOnAwake.boolValue, customSkin, "Invoke On Awake");
                    enableSaving.boolValue =
                        MUIPEditorHandler.DrawToggle(enableSaving.boolValue, customSkin, "Save Value");

                    if (enableSaving.boolValue)
                    {
