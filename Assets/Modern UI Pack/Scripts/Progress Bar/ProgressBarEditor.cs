            ...
            switch (currentTab)
            {
                case 0:
                    ...
                    EditorGUILayout.LabelField(new GUIContent("Current Percent"), customSkin.FindStyle("Text"),
                        GUILayout.Width(100));
                    pbTarget.currentPercent = EditorGUILayout.Slider(pbTarget.currentPercent, minValue.floatValue,
                        maxValue.floatValue);
                    ...
                    maxValue.floatValue = EditorGUILayout.Slider(maxValue.floatValue, minValue.floatValue + 1,
                        valueLimit.floatValue);

                    GUILayout.EndHorizontal();
                    GUILayout.Space(2);
                    EditorGUILayout.HelpBox(
                        "You can increase the max value limit by changing 'Value Limit' in the settings tab.",
                        MessageType.Info);
                ...
                case 2:
                    ...
                    GUILayout.Space(-3);
                    addPrefix.boolValue =
                        MUIPEditorHandler.DrawTogglePlain(addPrefix.boolValue, customSkin, "Add Prefix");
                    GUILayout.Space(3);
                    ...
                    GUILayout.Space(-3);
                    addSuffix.boolValue =
                        MUIPEditorHandler.DrawTogglePlain(addSuffix.boolValue, customSkin, "Add Suffix");
                    GUILayout.Space(3);
                    ...
                    EditorGUILayout.LabelField(new GUIContent("Value Limit"), customSkin.FindStyle("Text"),
                        GUILayout.Width(80));
                    EditorGUILayout.PropertyField(valueLimit, new GUIContent(""));

                    if (valueLimit.floatValue <= minValue.floatValue)
                        valueLimit.floatValue = minValue.floatValue + 1;
                    ...
                    {
                        EditorGUILayout.HelpBox(
                            "This object is connected with UI Manager. Some parameters (such as colors, "
                        ...
                        if (GUILayout.Button("Disable UI Manager Connection", customSkin.button))
                            if (EditorUtility.DisplayDialog("Modern UI Pack",
                                ...
                                {
                                    Debug.LogError("<b>[Progress Bar]</b> Failed to delete UI Manager connection.",
                                        this);
                                }
                    }

                    else if (tempFilledUIM != null && pbTarget.isLooped)
                    {
