...
namespace Michsky.MUIP
{
    ...
    public class CustomDropdownEditor : Editor
    {
        ...
        private void OnEnable()
        {
            ...
            try
            {
                tempUIM = dTarget.GetComponent<UIManagerDropdown>();
            }
            catch { }
            ...
            if (dTarget.selectedItemIndex > dTarget.items.Count - 1)
                dTarget.selectedItemIndex = 0;
        }
            ...
            switch (currentTab)
            {
                case 0:
                    ...
                    if (Application.isPlaying == false && dTarget.items.Count != 0)
                    {
                        ...
                        EditorGUILayout.LabelField(new GUIContent(dTarget.items[selectedItemIndex.intValue].itemName),
                        ...
                        selectedItemIndex.intValue =
                            EditorGUILayout.IntSlider(selectedItemIndex.intValue, 0, dTarget.items.Count - 1);

                        GUILayout.EndVertical();
                    }

                    else if (Application.isPlaying && dTarget.items.Count != 0)
                    {
                        GUILayout.BeginVertical(EditorStyles.helpBox);
                        GUILayout.BeginHorizontal();
                        GUI.enabled = false;
                        ...
                        EditorGUILayout.LabelField(new GUIContent("Current Item:"), customSkin.FindStyle("Text"),
                            GUILayout.Width(74));
                        EditorGUILayout.LabelField(new GUIContent(dTarget.items[dTarget.selectedItemIndex].itemName),
                            customSkin.FindStyle("Text"));
                ...
                case 2:
                    ...
                    enableIcon.boolValue =
                        MUIPEditorHandler.DrawToggle(enableIcon.boolValue, customSkin, "Enable Header Icon");
                    enableScrollbar.boolValue =
                        MUIPEditorHandler.DrawToggle(enableScrollbar.boolValue, customSkin, "Enable Scrollbar");
                    extendEvents.boolValue =
                        MUIPEditorHandler.DrawToggle(extendEvents.boolValue, customSkin, "Extend Events");
                    MUIPEditorHandler.DrawPropertyCW(itemSpacing, customSkin, "Item Spacing", 90);

                    GUILayout.BeginVertical(EditorStyles.helpBox);
                    GUILayout.BeginHorizontal();
                    EditorGUILayout.LabelField(new GUIContent("Item Padding"), customSkin.FindStyle("Text"),
                        GUILayout.Width(90));
                    GUILayout.EndHorizontal();
                    EditorGUI.indentLevel = 1;
                    ...
                    initAtStart.boolValue =
                        MUIPEditorHandler.DrawToggle(initAtStart.boolValue, customSkin, "Initialize At Start");
                    invokeAtStart.boolValue =
                        MUIPEditorHandler.DrawToggle(invokeAtStart.boolValue, customSkin, "Invoke At Start");
                    ...
                    else
                    {
                        if (enableIcon.boolValue)
                        {
                            ...
                            EditorGUILayout.HelpBox(
                                "'Selected Image' is not assigned. Go to Resources tab and assign the correct variable.",
                                MessageType.Error);
                            GUILayout.EndHorizontal();
                        }
                    }

                    enableTrigger.boolValue =
                        MUIPEditorHandler.DrawToggle(enableTrigger.boolValue, customSkin, "Enable Trigger");
                    if (enableTrigger.boolValue && dTarget.triggerObject == null)
                        EditorGUILayout.HelpBox("'Trigger Object' is missing from the resources.", MessageType.Warning);

                    setHighPriority.boolValue =
                        MUIPEditorHandler.DrawToggle(setHighPriority.boolValue, customSkin, "Set High Priority");
                    if (setHighPriority.boolValue)
                        EditorGUILayout.HelpBox(
                            "Set High Priority; renders the content above all objects when the dropdown is open.",
                            MessageType.Info);

                    outOnPointerExit.boolValue = MUIPEditorHandler.DrawToggle(outOnPointerExit.boolValue, customSkin,
                        "Out On Pointer Exit");

                    GUILayout.BeginVertical(EditorStyles.helpBox);
                    GUILayout.Space(-3);

                    enableDropdownSounds.boolValue = MUIPEditorHandler.DrawTogglePlain(enableDropdownSounds.boolValue,
                        customSkin, "Enable Dropdown Sounds");
                    ...
                    if (enableDropdownSounds.boolValue)
                    {
                        ...
                        useHoverSound.boolValue = MUIPEditorHandler.DrawTogglePlain(useHoverSound.boolValue, customSkin,
                            "Enable Hover Sound");
                        ...
                        useClickSound.boolValue = MUIPEditorHandler.DrawTogglePlain(useClickSound.boolValue, customSkin,
                            "Enable Click Sound");
                        ...
                        {
                            EditorGUILayout.HelpBox(
                                "'Sound Source' is not assigned. Go to Resources tab or click the button to create a new audio source.",
                                MessageType.Warning);

                            if (GUILayout.Button("+ Create a new one", customSkin.button))
                            {
                                dTarget.soundSource =
                                    dTarget.gameObject.AddComponent(typeof(AudioSource)) as AudioSource;
                                currentTab = 2;
                            }
                        }
                    }
                    ...
                    saveSelected.boolValue =
                        MUIPEditorHandler.DrawTogglePlain(saveSelected.boolValue, customSkin, "Save Selected");
                    ...
                    if (tempUIM != null)
                    {
                        ...
                        tempUIM.overrideColors =
                            MUIPEditorHandler.DrawToggle(tempUIM.overrideColors, customSkin, "Override Colors");
                        tempUIM.overrideFonts =
                            MUIPEditorHandler.DrawToggle(tempUIM.overrideFonts, customSkin, "Override Fonts");
                        ...
                        if (GUILayout.Button("Disable UI Manager Connection", customSkin.button))
                            if (EditorUtility.DisplayDialog("Modern UI Pack",
                                ...
                                {
                                    Debug.LogError("<b>[Dropdown]</b> Failed to delete UI Manager connection.", this);
                                }
                    }
